 ...
 * @param {Boolean} options.sendInitComment   Option to disable initial sse-comment. Defaults to true
 */
var SseChannel = function(options) {
    ...
    this.sendInitComment = (typeof opts.sendInitComment === 'undefined' ? true : opts.sendInitComment);
...
SseChannel.prototype.addClient = function(req, res, callback) {
    ...
    if (this.cors && this.cors(req, res)) {
        var err = res.statusCode === 403 ? new Error('CORS mismatch') : null;
        if (callback) {
    ...
    initializeConnection({
        ...
        sendInitComment: this.sendInitComment
    });
 ...
 * @param {Boolean}  opts.sendInitComment   Whether to send a ":ok" comment or not
 */
